...
int main()
{
	...
	//я задолбалась, поэтому сортировать будем пузырьком
	...
	tree_id = (int*)malloc((num_vertixes+1)* sizeof(int)); //массив, содержащий принадлежность вершин к одному "поддереву" в процессе построения MST
